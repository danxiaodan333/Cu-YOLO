...
from .atteniton import EfficientAttention, TripletAttention
from .conv import Conv, DWConv, GhostConv, LightConv, RepConv, autopad
from .rep_block import DiverseBranchBlock
from .transformer import TransformerBlock
from timm.models.layers import DropPath
__all__ = ('DFL', 'HGBlock', 'HGStem', 'SPP', 'SPPF', 'C1', 'C2', 'C3', 'C2f', 'C3x', 'C3TR', 'C3Ghost',
           'GhostBottleneck', 'Bottleneck', 'BottleneckCSP', 'Proto', 'RepC3', 'C2f_Faster', 'C2f_ODConv','Fusion',
           'VoVGSCSP', 'VoVGSCSPC', 'GSConv', 'C2f_Faster_EMA','C2f_CloAtt','Partial_conv3')
...
###################### PConv  ####     end#############################
# class EMAbottleneck(Bottleneck):
#        def __init__(self, c1, c2, shortcut=True, g=1, e=0.5,channels=6, factor=8):
...
#     def __init__(self,c1,c2,n=1,shortcut=False, g=1, e=0.5):
#         super().__init__(c1, c2, n, shortcut, g, e)
#         self.m = nn.ModuleList(EMAbottleneck(self.c, self.c, shortcut, g, e=1.0) for _ in range(n))
...
class C2f_SE(C2f):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.ModuleList(SE_Bootleneck(self.c, self.c, shortcut, g, e=1.0) for _ in range(n))


class EMA_Bootleneck(nn.Module):
    def __init__(self, c1, c2, shortcut=True, g=1, k=(3, 3), e=0.5):
        super().__init__()
        c_ = int(c2 * e)
        self.cv1 = Conv(c1, c_, k[0], 1)
...
class C2f_ema(C2f):
    ...
    def __init__(self, c1, c2, n=1, shortcut=True, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        c_ = int(c2 * e)  # hidden channels
        self.m = nn.ModuleList(EMA_Bootleneck(self.c, self.c, shortcut, g, e=1.0) for _ in range(n))

class Faster_Block_Triplet(nn.Module):
    def __init__(self,
                 inc,
                 dim,
                 n_div=4,
        ...
        self.attention = TripletAttention(dim)
...
class C2f_Faster_Triplet(C2f):
    def __init__(self, c1, c2, n=1, shortcut=False, g=1, e=0.5):
        super().__init__(c1, c2, n, shortcut, g, e)
        self.m = nn.ModuleList(Faster_Block_Triplet(self.c, self.c) for _ in range(n))
